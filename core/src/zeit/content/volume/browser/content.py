...
class CommonListRepresentation(BaseListRepresentation):
    ...
    def urgent(self):
        ...
        if IContentWorkflow(content).urgent:
            return True
        return False
    ...
    def seo_optimized(self):
        content = zeit.cms.interfaces.ICMSContent(self.context.uniqueId)
        if IContentWorkflow(content).seo_optimized:
            return True
        return False
    ...
    def searchableText(self):
        items = []
        for name in ('author', 'title', 'supertitle', 'subtitle', 'byline',
                     'ressort', 'volume', 'page', 'urgent', 'seo_optimized',
                     'year', '__name__'):
            try:
...
class CheckColumn(GetterColumn):
...
class DatetimeColumn(GetterColumn):
    ...
    def cell_formatter(self, value, item, formatter):
        if not value:
            return u''
        tzinfo = zope.interface.common.idatetime.ITZInfo(
    ...
    def content(self):
        ...
        for obj in sorted(
            ...
            if list_repr is None:
                logger.warning(
                    "Could not adapt %r to IListRepresentation", (obj, ))
            else:
