...
class IGalleryEntry(zope.interface.Interface):
    ...
    crops = zope.interface.Attribute('List of IGalleryEntry that are crops of this entry')

    is_crop_of = zope.schema.TextLine(
        title=_('Is a cropped image of'), required=False, readonly=True
    )
    ...
    # accessed (see #8858)
    text = zeit.cms.content.field.XMLTree(title=_('Text'), required=False)

    layout = zope.schema.Choice(
        title=_('Layout'), required=False, source=zeit.content.gallery.source.LayoutSource()
    )
