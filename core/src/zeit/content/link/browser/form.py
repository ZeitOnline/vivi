...
class Base(zeit.push.browser.form.SocialBase, zeit.push.browser.form.MobileBase):
    ...
    option_group = gocept.form.grouped.Fields(
        _('Options'), ('channels', 'serie', 'access', 'keywords'), css_class='column-right'
    )

    marketing_group = gocept.form.grouped.Fields(
        _('Marketing'), ('cap_title', 'banner_id'), css_class='column-right'
    )

    link_group = gocept.form.grouped.Fields(
        _('Link'), ('status_code', 'target', 'nofollow'), css_class='column-right'
    )

    # Make link specific field groups for other views
    link_field_groups = (metadata_group, option_group, marketing_group, teaser_group, link_group)

    form_fields = zope.formlib.form.FormFields(
        zeit.content.link.interfaces.ILink, zeit.content.image.interfaces.IImages
    ).select(
        *list(
            itertools.chain.from_iterable([group.get_field_names() for group in link_field_groups])
        )
    )
