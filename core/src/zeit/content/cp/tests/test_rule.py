...
"""
        ...
        self.teaser.insert(0, zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/testcontent'))
        s = r.apply(self.teaser, IRuleGlobs(self.teaser))
...
class RulesManagerTest(zeit.content.cp.testing.FunctionalTestCase):
    def setUp(self):
        ...
        super().setUp()
        self.rm = zope.component.getUtility(zeit.edit.interfaces.IRulesManager)

    def _set_rules(self, filename):
        zope.app.appsetup.product._configs['zeit.edit']['rules-url'] = 'file://%s' % (
            importlib.resources.files(__package__) / 'fixtures' / filename
        )
