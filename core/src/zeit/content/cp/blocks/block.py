...
class VisibleMixin:
    visible = zeit.cms.content.property.ObjectPathAttributeProperty(
        '.', 'visible', zeit.content.cp.interfaces.IElement['visible']
    )

    def __init__(self, context, xml):
        super().__init__(context, xml)
        if 'visible' not in self.xml.attrib:
            self.visible = zeit.content.cp.interfaces.IElement['visible'].default
...
class Block(VisibleMixin, zeit.edit.block.SimpleElement):
    ...
    supertitle = zeit.cms.content.property.ObjectPathAttributeProperty('.', 'supertitle')
    supertitle_url = zeit.cms.content.property.ObjectPathAttributeProperty('.', 'supertitle_url')
    ...
    volatile = zeit.cms.content.property.ObjectPathAttributeProperty(
        '.', 'volatile', zeit.content.cp.interfaces.IBlock['volatile'], use_default=True
    )

    # XXX Is this attribute used anywhere? Imho it's dead code.
    publisher = zeit.cms.content.property.ObjectPathAttributeProperty('.', 'publisher')
    publisher_url = zeit.cms.content.property.ObjectPathAttributeProperty('.', 'publisher_url')
    ...
    def type_title(self):
        ...
        module_config = zeit.content.cp.layout.MODULE_CONFIGS(None).find(self.type)
        if module_config:
