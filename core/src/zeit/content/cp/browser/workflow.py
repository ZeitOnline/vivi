...
def is_published_and_has_permission(form, action):
    return zeit.workflow.browser.form.is_published(
        form, action
    ) and form.request.interaction.checkPermission('zeit.content.cp.Retract', form.context)


@zope.component.adapter(
    zeit.content.cp.interfaces.ICenterPage, zeit.cms.browser.interfaces.ICMSLayer
)
class CenterPageWorkflowForm(zeit.workflow.browser.form.WorkflowForm):
    ...
    def get_error_message(self, mapping):
        return _('Could not publish ${id} since it has validation errors.', mapping=mapping)
