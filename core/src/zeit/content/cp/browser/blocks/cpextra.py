...
class CPExtraView:
    ...
    def cpextra_title(self):
        if not self.context.cpextra:
            return None
        source = zeit.content.cp.interfaces.ICPExtraBlock['cpextra'].source(self.context)
...
class EditProperties(zeit.content.cp.browser.blocks.block.EditCommon):
    form_fields = zope.formlib.form.FormFields(zeit.content.cp.interfaces.ICPExtraBlock).omit(
