...
class TestPlaylist(zeit.content.cp.testing.BrowserTestCase):
    def setUp(self):
        ...
        self.playlist = self.centerpage.body['lead'].create_item('playlist')
        self.repository['centerpage'] = self.centerpage
        self.repository['my-playlist'] = Playlist()

        self.browser.open('http://localhost/++skin++vivi/repository/centerpage/@@checkout')
    ...
    def test_can_create_playlist_block_via_drag_n_drop_from_sidebar(self):
        ...
        b.open('body/informatives/@@landing-zone-drop-module?block_type=playlist')
        b.open(self.content_url)
        self.assertEqual(2, b.contents.count('type-playlist'))

    def test_can_reference_a_playlist_via_drag_n_drop(self):
        [playlist_block] = self.repository['centerpage'].body['lead'].values()
        drop_url = 'body/lead/{}/@@drop?uniqueId={}'.format(
        ...
        b.open(self.content_url)
        b.getLink('Edit block properties', index=0).click()
        self.assertEqual('http://xml.zeit.de/my-playlist', b.getControl('Playlist').value)
    ...
    def test_cannot_drop_video(self):
        ...
        with self.assertRaises(urllib.error.HTTPError):
            b.open(drop_url)
        self.assertEllipsis('...Only playlists can be dropped on a playlist block...', b.contents)

    def test_can_reference_a_playlist_by_editing_common_properties(self):
        self.set_referenced_playlist()
        self.browser.open('@@checkin')
        [playlist_block] = self.repository['centerpage'].body['lead'].values()
        self.assertEqual(self.repository['my-playlist'], playlist_block.referenced_playlist)
...
<container cp:type="playlist"...>
    ...
    def test_playlist_is_part_of_cp_content(self):
        ...
        self.assertEqual(
            ...
            list(zeit.edit.interfaces.IElementReferences(self.repository['centerpage'])),
        )
