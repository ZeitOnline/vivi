...
class FormBase:
    ...
    head_fields = gocept.form.grouped.Fields(
        _('Head'), ('ressort', 'sub_ressort'), css_class='widgets-float column-left'
    )
    ...
    cpform_fields = zope.formlib.form.FormFields(
        zeit.content.cp.interfaces.ICenterPage, zeit.content.image.interfaces.IImages
    )
...
class AddForm(FormBase, zeit.cms.content.browser.form.CommonMetadataAddForm):
    title = _('Add centerpage')
    factory = zeit.content.cp.centerpage.CenterPage
    next_view = 'view.html'


class EditForm(FormBase, zeit.cms.content.browser.form.CommonMetadataEditForm):
    title = _('Edit centerpage')


class DisplayForm(FormBase, zeit.cms.content.browser.form.CommonMetadataDisplayForm):
...
class SEOCpForm:
    form_fields = (
        ...
        + zope.formlib.form.FormFields(zeit.content.cp.interfaces.ICpSEO).select(
