...
class EditorHelper:
    ...
    def mark_dirty(self, locator=None, status=True):
        ...
        self.selenium.runScript(editable + '.dirty = ' + str(status).lower() + ';')
    ...
    def create_block(self, block, wait_for_inline=False):
        ...
        if wait_for_inline:
            s.waitForElementPresent('css={0} form.inline-form.wired'.format(block_sel))
    ...
    def add_testcontent_to_clipboard(self):
        ...
        clip = clipboard['Clip']
        clipboard.addContent(clip, self.repository['testcontent'], 'testcontent', insert=True)
        transaction.commit()
...
class EditorTestCase(zeit.content.article.testing.SeleniumTestCase, EditorHelper):
    window_width = 1600
    window_height = 1000
