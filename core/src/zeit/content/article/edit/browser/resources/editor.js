...
zeit.content.article.Editable = gocept.Class.extend({
    ...
    // Orchestrates the conversion between paragraphs and lists.
    ...
    _find_editable_block: function(node) {
        var self = this;
    ...
    _is_block_element: function(node) {
        var self = this;
    ...
    // Collects all block elements within the selection range. This handles both
    // single-block and multi-block selections.
    _get_selected_blocks: function(range) {
        var self = this;
    ...
    _all_blocks_are_lists: function(blocks) {
    ...
    _convert_lists_to_paragraphs: function(blocks) {
    ...
    // Converts all selected blocks to a single list. Each block becomes a <li> item.
    // Existing lists have their items extracted and merged into the new list.
    _convert_blocks_to_list: function(blocks, listType, range) {
        var self = this;
    ...
    _extract_list_items_from_block: function(block, listType) {
