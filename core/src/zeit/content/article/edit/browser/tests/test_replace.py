...
class FindReplaceTest(zeit.content.article.edit.browser.testing.EditorTestCase):
    def add_article(self):
        super().add_article()
        below_body = '#edit-form-recensions .edit-bar .fold-link'
        self.eval('document.querySelector("%s").scrollIntoView()' % below_body)
    ...
    def test_finding_text_works_accross_non_text_blocks(self):
        ...
        self.eval('document.querySelector("%s").scrollIntoView()' % para.replace('css=', ''))
        s.clickAt(para, '5,5')
        ...
        self.wait_for_condition('window.getSelection().getRangeAt(0).startOffset == 8')
        s.click('jquery=button:contains(Weiter)')
        self.wait_for_condition('window.getSelection().getRangeAt(0).startOffset == 4')
    ...
    def test_asks_for_confirmation_to_wrap_around_at_end(self):
        ...
        s.click('jquery=button:contains(Weiter)')
        s.click('jquery=button:contains(Weiter)')
        s.waitForConfirmation('Das Textende wurde erreicht. Suche am Textanfang fortsetzen?')
        self.wait_for_condition('window.getSelection().getRangeAt(0).startOffset == 0')
    ...
    def test_asks_for_confirmation_to_wrap_around_at_beginning(self):
        ...
        s.click('jquery=button:contains(Zurück)')
        s.click('jquery=button:contains(Zurück)')
        s.waitForConfirmation('Der Textanfang wurde erreicht. Suche am Textende fortsetzen?')
        self.wait_for_condition('window.getSelection().getRangeAt(0).startOffset == 8')

    @unittest.skip(
        'Browser focuses funky things after one clicks cancel,'
    ...
    def test_closes_replace_dialog_on_cancelled_wrapping_confirmation(self):
        ...
        self.assertEqual(
            'Das Textende wurde erreicht. Suche am Textanfang fortsetzen?', confirmation.text
        )
