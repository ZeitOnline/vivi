...
class RecipeCategoriesTest(zeit.content.article.edit.browser.testing.BrowserTestCase):
    def test_genre_should_toggle_speechbert_value_at_some_values(self):
        b = self.browser
        genre = b.getControl(name='metadata-genre.genre')
        # Starting with article with 'no genre' option
        genre.value = ''
        b.getControl(name='metadata-genre.actions.apply').click()
        b.open('@@contents')
        sb_cb = b.xpath('//input[@id="options-audio-speechbert.audio_speechbert"]')[0]
    ...
    def test_genre_should_toggle_recipe_categories(self):
        ...
        b.getControl(name='metadata-genre.actions.apply').click()
        b.open('@@contents')
        self.assertTrue('<div class="recipe-categories-widget"' not in (b.contents))
        # We still expect an empty fieldset
        self.assertTrue('<fieldset id="form-recipe-categories" /></form>' in (b.contents))
...
class FormLoader(zeit.content.article.edit.browser.testing.EditorTestCase):
    def test_recipe_categories_should_be_organizable(self):
        ...
        self.assertEqual(s.getCssCount('css=li.recipe-category__item'), 2)  # not 3
