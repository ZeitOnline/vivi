...
class Form(zeit.content.article.edit.browser.testing.BrowserTestCase):
    def test_api_call_result_renders_correct_select_form(self):
        ...
        self.assertEqual('radio', brwsr.getControl('Video 1')._elem.get('type'))
        self.assertEqual('crid://daserste.de/crid1', brwsr.getControl('Video 1').value)
        self.assertEqual('crid://daserste.de/crid3', brwsr.getControl('Video 3').value)
        self.assertTrue(
    ...
    def test_api_call_succeeds_with_basic_informations_and_different_datetime_format(self):  # noqa
        api = zope.component.getUtility(zeit.content.article.edit.interfaces.IVideoTagesschauAPI)
        api.request_videos.return_value = json.loads(MOCKDEFAULTEMPTYUNIMPORTANT)
        self.get_article(with_block='videotagesschau')
        brwsr = self.browser
        brwsr.open('editable-body/blockname/@@edit-videotagesschau?show_form=1')
        self.assertEqual(len(brwsr.xpath('//input[@type="radio"]')), 1)
...
class Form2(zeit.content.article.testing.FunctionalTestCase):
    def test_correct_payload_is_extracted_from_article_attributes(self):
        ...
        body = (
            '<division><p>Das ist Tagesschau relevant.</p>'
        ...
        payload = api._prepare_payload(article)
        self.assertEqual(
            'localhost/artikel/pfad/tagesschaurelevanter-artikel', payload['article_uri']
        )
