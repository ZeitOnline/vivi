...
class StackableStorage(gcp_storage_emulator.storage.Storage):
    """Storage that uses a stack of in-memory FS objects,
    writes go into the last entry, reads fall back to previous entries.
    This allows undoing a whole set of writes in one fell swoop.

    Note that deleting a file does NOT stack properly with this implementation.
    Instead of marking it deleted in the last stack entry, it will delete the
