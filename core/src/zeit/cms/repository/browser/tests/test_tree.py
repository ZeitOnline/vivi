...
class TestTree(zeit.cms.testing.SeleniumTestCase):
    ...
    def test_tree_keeps_state(self):
        ...
        s.waitForElementPresent('//li[@uniqueid="http://xml.zeit.de/online/2007/"]')
...
class TreeURLTest(zeit.cms.testing.ZeitCmsTestCase):
    def test_handles_unicode_in_uniqueIds(self):
        ...
        view = zeit.cms.repository.browser.repository.HTMLTree(UNUSED, request, UNUSED, UNUSED)
        with mock.patch('zeit.cms.repository.interfaces.IUserPreferences') as prefs:
            with mock.patch('zeit.cms.browser.tree.Tree.treeState', new=['http://xml.zeit.de/bär']):
                prefs().get_hidden_containers.return_value = ['http://xml.zeit.de/föö']
                self.assertEqual(
