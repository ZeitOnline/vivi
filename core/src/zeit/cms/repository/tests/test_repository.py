...
class TestConflicts(zeit.cms.testing.ZeitCmsTestCase):
    ...
    def test_conflict_on_add_content(self):
        self.assertRaises(
            zeit.cms.repository.interfaces.ConflictError, self.repository.addContent, self.res
        )
...
class LiveURLToContent(unittest.TestCase):
    ...
    def test_page_marker_is_chopped_off(self):
        live_url_to_content('http://www.zeit.de/online/2007/01/Somalia/seite-5')
        self.cmscontent.assert_called_with('http://xml.zeit.de/online/2007/01/Somalia', None)

    def test_komplettansicht_is_chopped_off(self):
        live_url_to_content('http://www.zeit.de/online/2007/01/Somalia/komplettansicht')
        self.cmscontent.assert_called_with('http://xml.zeit.de/online/2007/01/Somalia', None)
...
class ViviURLToContent(unittest.TestCase):
    ...
    def test_vivi_url_is_replaced_by_xml_url(self):
        ...
        self.cmscontent.assert_called_with('http://xml.zeit.de/2007/politik', None)

    def test_vivi_view_names_are_removed(self):
        vivi_url_to_content('http://vivi.zeit.de/repository/2007/politik/@@something.html')
        self.cmscontent.assert_called_with('http://xml.zeit.de/2007/politik', None)

    def test_url_not_in_repository_returns_none(self):
        self.assertIsNone(vivi_url_to_content('http://vivi.zeit.de/workingcopy/user/Somalia'))
...
class UniqueIdToContentIntegration(zeit.cms.testing.ZeitCmsTestCase):
    def test_xml_zeit_de_is_translated_to_content_object(self):
        self.assertIsInstance(ICMSContent('http://xml.zeit.de/testcontent'), ExampleContentType)
    ...
    def test_no_interaction_ignores_workingcopy(self):
        ...
        self.assertEqual(
            ICMSContent('http://xml.zeit.de/testcontent'),
            zeit.cms.cmscontent.resolve_wc_or_repository('http://xml.zeit.de/testcontent'),
        )
...
class RepositoryTest(zeit.cms.testing.ZeitCmsTestCase):
    ...
    def test_cache_is_cleared_on_transaction(self):
        ...
        with self.assertRaises(AttributeError):
            self.repository['kultur']._v_local_unique_map  # noqa: B018
...
class ContentBaseTest(zeit.cms.testing.ZeitCmsTestCase):
    ...
    def test_root_folder_is_repository(self):
        self.assertEqual(
            self.repository, self.repository.getContent('http://xml.zeit.de/testcontent').__parent__
        )

    def test_result_has_name(self):
        self.assertEqual(
            'testcontent', self.repository.getContent('http://xml.zeit.de/testcontent').__name__
        )
