    ...
    def setUp(self):
        with self['rootFolder'](self['zodbDB-layer']) as root:
            with zeit.cms.testing.site(root):
                ...
                self['principalfolder']['user'] = InternalPrincipal('user', 'userpw', 'Testuser')
                ...
                auth.credentialsPlugins = ('No Challenge if Authenticated', 'Session Credentials')
    ...
    def tearDown(self):
        ...
        with self['rootFolder'](self['zodbDB-layer']) as root:
            ...
            site_manager.unregisterUtility(
                site_manager['authentication'], zope.authentication.interfaces.IAuthentication
            )
...
class SSOTest(zeit.cms.testing.BrowserTestCase):
    ...
    def jwt_decode(self, value):
        return jwt.decode(
            value.encode('ascii'),
            (importlib.resources.files('zeit.cms.tests') / 'sso-public.pem').read_text('ascii'),
            algorithms='RS256',
        )
    ...
    def test_have_permission_sets_cookie(self):
        ...
        self.assertEqual('http://localhost/++skin++vivi/repository/online/2008/26', b.url)
