...
from zeit.cms.browser.widget import (
    ConvertingRestructuredTextWidget,
    DropObjectWidget,
...
class TestObjectDetails(zeit.cms.testing.ZeitCmsBrowserTestCase):
    ...
    def test_should_contain_teaser_title(self):
        ...
        self.assert_ellipsis('...<div class="teaser_title">test title</div>...')
    ...
    def test_should_contain_workflow_information(self):
        self.browser.open('@@object-details')
        self.assert_ellipsis('...class="publish-state not-published"...')

    def test_should_contain_type_identifier(self):
        self.browser.open('@@object-details')
        self.assert_ellipsis('...class="object-details type-testcontenttype"...')
...
class TestObjectDetailsJavascript(zeit.cms.testing.SeleniumTestCase):
    ...
    def test_icon_is_draggable_as_content_object(self):
        self.open('/@@/zeit.cms.browser.tests.fixtures/contenticondrag.html')
        s = self.selenium
        s.waitForElementPresent('css=#result .content-icon')
        s.dragAndDropToObject('css=#result .content-icon .uniqueId', 'id=testwidget', '10,10')
        s.waitForValue('name=testwidget', 'http://xml.zeit.de/testcontent')
...
class TestObjectSequenceWidget(zeit.cms.testing.ZeitCmsTestCase):
    ...
    def test_uniqueId_not_in_source_should_raise(self):
        ...
        context.__name__ = 'foo'
        source = FakeSource()
        widget = zeit.cms.browser.widget.DropObjectWidget(context, source, request=mock.Mock())
        self.assertRaises(
...
class TestObjectSequenceWidgetIntegration(
    zeit.cms.testing.ZeitCmsTestCase, zeit.cms.testing.BrowserAssertions
):
    def setUp(self):
        super().setUp()
    ...
    def get_widget(self, field=None):
        ...
        zope.interface.alsoProvides(request, zeit.cms.browser.interfaces.IGlobalSearchLayer)
        widget = zope.component.getMultiAdapter(
            (field, request), zope.formlib.interfaces.IInputWidget
        )
    ...
    def test_accepted_types_is_escaped_for_javascript(self):
        field = self.get_field()
        widget = self.get_widget(field)
        with mock.patch.object(field.value_type.source, 'get_check_types') as types:
...
class TestObjectSequenceWidgetJavascript(zeit.cms.testing.SeleniumTestCase):
    ...
    def test_widget_should_render_note_about_new_items(self):
        s = self.selenium
        s.waitForTextPresent('Ziehen Sie Inhalte hierher um sie zu verknüpfen.')
    ...
    def test_drop_should_create_hidden_field_with_unique_id(self):
        ...
        s.assertValue("//input[@name='testwidget.0']", 'http://xml.zeit.de/testcontent')
    ...
    def test_entering_uid_should_increase_count(self):
        s = self.selenium
        s.assertValue("//input[@name='testwidget.count']", '0')
        s.type("//input[@name='testwidget.url']", 'http://xml.zeit.de/testcontent')
    ...
    def test_sorting_should_update_hidden_field_indexes(self):
        ...
        s.waitForCssCount('css=.object-details', 2)
        s.assertValue("//input[@name='testwidget.0']", 'http://xml.zeit.de/testcontent')
        s.assertValue("//input[@name='testwidget.1']", 'http://xml.zeit.de/2007')
        s.dragAndDropToObject('jquery=li.element[index = 0]', 'jquery=li.element[index = 1]')
        s.assertValue("//input[@name='testwidget.0']", 'http://xml.zeit.de/2007')
        s.assertValue("//input[@name='testwidget.1']", 'http://xml.zeit.de/testcontent')
...
"""
        ...
        self.assertEqual(['foo'], self.eval('zeit.cms._activate_objectbrowser_arg'))
...
def teardown_mydetails():
    with zeit.cms.testing.site(None):
        zope.component.getSiteManager().unregisterAdapter(
            required=(zeit.cms.interfaces.ICMSContent, zeit.cms.browser.interfaces.ICMSLayer),
...
class ObjectWidgetDetailViews(zeit.cms.testing.SeleniumTestCase):
    ...
    def test_object_sequence_widgets_use_their_configured_views(self):
        ...
        self.execute("zeit.cms.test_widget2.add('http://xml.zeit.de/testcontent');")
...
class TestObjectSequenceWidgetAutocompleteJavascript(zeit.cms.testing.SeleniumTestCase):
    layer = zeit.cms.testing.WEBDRIVER_LAYER
    ...
    def test_selecting_autocomplete_should_add_object(self):
        ...
        s.assertValue('id=testwidget.0', 'http://xml.zeit.de/autoren/A/Test_Autor/index')
...
class TestDropObjectWidgetFoo(zeit.cms.testing.SeleniumTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.open('/@@/zeit.cms.browser.tests.fixtures/dropobjectwidget.html')
...
class TestDropObjectWidgetAccept(zeit.cms.testing.SeleniumTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.open('/@@/zeit.cms.browser.tests.fixtures/dropobjectwidget-accept.html')
...
class TestDropObjectWidgetIntegration(zeit.cms.testing.ZeitCmsTestCase):
    def setUp(self):
        super().setUp()
        zope.security.management.endInteraction()
    ...
    def test_widget_should_be_available_with_search(self):
        ...
        zope.interface.alsoProvides(request, zeit.cms.browser.interfaces.IGlobalSearchLayer)
        widget = zope.component.getMultiAdapter(
            (choice, request), zope.formlib.interfaces.IInputWidget
        )
    ...
    def test_widget_detail_view_name_can_be_configured(self):
        choice = self.get_choice()
        request = zope.publisher.browser.TestRequest()
        widget = zeit.cms.browser.widget.DropObjectWidget(choice, choice.source, request)
        widget.detail_view_name = '@@mydetails'
        self.assertEllipsis(
...
class TestDropObjectWidget(zeit.cms.testing.ZeitCmsTestCase):
    def test_setting_invalid_uniqueId_should_raise(self):
        context = mock.Mock()
        context.__name__ = 'foo'
        widget = zeit.cms.browser.widget.DropObjectWidget(context, mock.Mock(), mock.Mock())
        self.assertRaises(
            zope.formlib.interfaces.ConversionError,
    ...
    def test_setting_valid_uniqueId_returns_content_object(self):
        ...
        widget = zeit.cms.browser.widget.DropObjectWidget(context, source, request=mock.Mock())
        self.assertEqual(
            self.repository['testcontent'], widget._toFieldValue('http://xml.zeit.de/testcontent')
        )
...
class TestObjectSequenceDisplayWidgetIntegration(
    ...
    def get_widget(self):
        ...
        widget = zope.component.getMultiAdapter(
            (field, request), zope.formlib.interfaces.IDisplayWidget
        )
        return widget

    def get_content(self):
        return zeit.cms.interfaces.ICMSContent('http://xml.zeit.de/testcontent')
    ...
    def test_should_render_details_for_referenced_items(self):
        ...
        with zeit.cms.testing.interaction():
            self.assert_ellipsis('...<div class="object-details...teaser_title...', widget())
    ...
    def test_should_use_configured_detail_views(self):
        ...
        widget._data = (content,)
        with zeit.cms.testing.interaction():
            self.assert_ellipsis('...<div...id="field."...mydetails...', widget())
...
class TestReferenceSequenceWidget(zeit.cms.testing.ZeitCmsTestCase):
    def setUp(self):
        super().setUp()
        ExampleContentType.references = zeit.cms.content.reference.ReferenceProperty(
        ...
        self.field = zeit.cms.content.interfaces.ReferenceField(source=FakeSource())
    ...
    def test_unique_id_of_reference_returns_existing_reference(self):
        ...
        widget = ReferenceSequenceWidget(self.field, self.field.source, request=mock.Mock())
        self.assertEqual(
    ...
    def test_content_not_in_source_should_raise(self):
        ...
        widget = ReferenceSequenceWidget(self.field, self.field.vocabulary, request=mock.Mock())
        self.assertRaises(
            zope.formlib.interfaces.ConversionError,
...
class RestructuredTextWidgetTest(zeit.cms.testing.ZeitCmsTestCase):
    def setUp(self):
        super().setUp()
        request = zope.publisher.browser.TestRequest(skin=zeit.cms.browser.interfaces.ICMSSkin)
...
...""",
    ...
    def test_text_starting_with_http_is_rendered_as_link(self):
        ...
        self.assertEllipsis(
            '...<a...href="http://example.com">http://example.com</a>...', self.widget()
        )
...
class RestructuredTextWidgetJavascriptTest(zeit.cms.testing.SeleniumTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.open('/@@/zeit.cms.browser.tests.fixtures/restructuredtext.html')
...
class ConvertingRestructuredTextWidgetTest(zeit.cms.testing.ZeitCmsTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_converts_input_to_html(self):
        ...
        self.assertEqual('<p><strong>foo</strong></p>\n', self.widget.getInputValue())

    @unittest.skipUnless(os.path.exists('/usr/bin/pandoc'), 'pandoc not available')
    ...
    def test_renders_html_when_pandoc_is_not_available(self):
        ...
        with mock.patch.dict(os.environ, {'PATH': ''}):
            self.assertEqual('<strong>foo</strong>', self.widget._getFormValue())
...
class MarkdownWidgetTest(zeit.cms.testing.ZeitCmsTestCase):
    ...
    def test_converts_input_to_html(self):
        ...
        self.assertEqual('<p><strong>umläut</strong></p>', self.widget.getInputValue())
