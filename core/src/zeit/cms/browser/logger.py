...
class JSONLog(zeit.cms.browser.view.JSON):
    ...
    def json(self):
        decoded = json.loads(self.request.bodyStream.read(int(self.request['CONTENT_LENGTH'])))
        ...
        user = (error_reporting_util._getUsername(self.request) or '').split(', ')
        if user:
