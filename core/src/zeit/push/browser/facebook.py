...
class IFacebookApp(zope.interface.Interface):
    app_id = zope.schema.TextLine(title=_('Facebook app id'))
    app_secret = zope.schema.TextLine(title=_('Facebook app secret'))
    page_name = zope.schema.TextLine(title=_('Facebook page name (e.g. "ZEIT ONLINE")'))


class TokenForm(zeit.cms.browser.form.FormBase, gocept.form.grouped.Form):
    form_fields = zope.formlib.form.FormFields(IFacebookApp)
    field_groups = (gocept.form.grouped.RemainingFields(_('')),)

    @zope.formlib.form.action(_('Create'), condition=zope.formlib.form.haveInputWidgets)
    def redirect_to_facebook(self, action, data):
        data['redirect_uri'] = self.url(self.context, 'generate-facebook-token')
...
class GenerateToken(zeit.cms.browser.view.Base):
    def __call__(self):
        ...
        if 'error' in r.text:
            ...
            raise SystemExit(1)
        self.page_token = [
            x['access_token'] for x in r.json()['data'] if x['name'] == self.settings['page_name']
        ][0]
