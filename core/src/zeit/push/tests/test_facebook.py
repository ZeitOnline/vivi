...
class FacebookTest(zeit.push.testing.TestCase):
    ...
    def setUp(self):
        ...
        self.api = zeit.push.facebook.Connection(os.environ['ZEIT_PUSH_FACEBOOK_URL'])
    ...
    def tearDown(self):
        for status in self.api._request('GET', '/me/feed', self.access_token)['data']:
            if self.nugget in status.get('message', ''):
                self.api._request('DELETE', f'/{status["id"]}', self.access_token)
        super().tearDown()
    ...
    def test_send_posts_status(self):
        ...
        for status in self.api._request('GET', '/me/feed', self.access_token)['data']:
            if self.nugget in status['message']:
                ...
                link = self.api._request('GET', f'/{status["id"]}/attachments', self.access_token)
