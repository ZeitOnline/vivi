...
def from_product_config():
    ...
    return Connection(config['twitter-application-id'], config['twitter-application-secret'])
...
class TwitterClient(tweepy.Client):
    ...
    def bearer_token(self):
        return zope.component.getUtility(ITwitterCredentials).access_token(self.account_name)
    ...
    def refresh_token(self):
        ...
        creds.update(self.account_name, new['access_token'], new['refresh_token'])
...
def create_access_token(argv=None):
    parser = argparse.ArgumentParser(description='Create Twitter access token')
