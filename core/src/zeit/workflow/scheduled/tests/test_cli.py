...
class ScheduledOperationsCLITest(zeit.workflow.testing.FunctionalTestCase):
    ...
    def test_execute_merges_collection_properties(self):
        import zeit.cms.content.interfaces

        with zeit.cms.checkout.helper.checked_out(self.content) as co:
            co.channels = (('Green', None),)
        transaction.commit()
        ...
        ops.add(
            'publish',
            pendulum.now('UTC').add(seconds=-1),
            property_changes={'channels': [['Hamburg', 'kultur']]},
        )
        transaction.commit()
    ...
    def test_execute_skips_collection_items_already_present(self):
        with zeit.cms.checkout.helper.checked_out(self.content) as co:
            co.channels = (('Hamburg', 'kultur'),)
        transaction.commit()
        ...
        content = self.repository['testcontent']
        self.assertEqual((('Hamburg', 'kultur'),), content.channels)

    def test_execute_skips_scalar_property_already_set(self):
        with zeit.cms.checkout.helper.checked_out(self.content) as co:
            co.access = 'abo'
        transaction.commit()
        ...
        content = self.repository['testcontent']
        self.assertEqual('abo', content.access)
