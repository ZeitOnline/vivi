...
class ScheduledOperationsTest(zeit.workflow.testing.FunctionalTestCase):
    ...
    def test_add_validates_operation_type(self):
        ops = IScheduledOperations(self.content)
        scheduled_time = pendulum.now('UTC').add(hours=1)

        with self.assertRaises(ValueError) as cm:
            ops.add('invalid_operation', scheduled_time)

        self.assertIn('Invalid operation', str(cm.exception))

    def test_add_validates_property_changes_is_dict(self):
        ops = IScheduledOperations(self.content)
        scheduled_time = pendulum.now('UTC').add(hours=1)

        with self.assertRaises(ValueError) as cm:
            ops.add('publish', scheduled_time, property_changes='not_a_dict')

        self.assertIn('property_changes must be a dict', str(cm.exception))
    ...
    def test_remove_raises_keyerror_for_nonexistent_operation(self):
        ops = IScheduledOperations(self.content)

        with self.assertRaises(KeyError):
            ops.remove('nonexistent-id')
    ...
    def test_update_raises_keyerror_for_nonexistent_operation(self):
        ops = IScheduledOperations(self.content)

        with self.assertRaises(KeyError):
            ops.update('nonexistent-id', scheduled_on=pendulum.now('UTC'))
    ...
    def test_get_raises_keyerror_for_nonexistent_operation(self):
        ops = IScheduledOperations(self.content)

        with self.assertRaises(KeyError):
            ops.get('nonexistent-id')
