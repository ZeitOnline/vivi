...
class ContentStatus(zope.viewlet.viewlet.ViewletBase):
    ...
    def update(self):
        self.workflow = zeit.workflow.interfaces.IContentWorkflow(self.context, None)
    ...
    def render(self):
        ...
        for name in ('edited', 'corrected'):
            ...
            short_title = zope.i18n.translate(field.title, context=self.request)[0]
            long_title = zope.i18n.translate(
                _('${title}: ${state}', mapping={'title': field.title, 'state': value_title}),
            ...
            states.append(
                '<span title=%s class="content-status %s">'
                '%s</span>'
                % (
                    xml.sax.saxutils.quoteattr(long_title),
                    class_,
