...
class ITimeBasedPublishing(zeit.cms.workflow.interfaces.IPublishInfo):
    ...
    released_from = zope.interface.Attribute('Easy access to release_period[0]')
    released_to = zope.interface.Attribute('Easy access to release_period[1]')
...
class IContentWorkflow(ITimeBasedPublishing):
    ...
    urgent = zope.schema.Bool(
        title=_('Urgent'),
        description=_(
            'Select for newsflashs or on a weekend to publish '
...
class IOldCMSStatus(zope.interface.Interface):
    ...
    status = zope.schema.TextLine(title='Status like old CMS (OK, or not present)', required=False)
