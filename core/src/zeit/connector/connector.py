...
from dav import davresource
from dav.davresource import DAVResource, DAVCollection, DAVFile, DAVError
...
class Connector(zope.thread.local):
    ...
    def __getitem__(self, id):
        ...
        return CachedResource(id, _id_splitlast(id)[1],
    ...
    def changeProperties(self, id, properties):
        ...
        davres = self._get_dav_resource(id)
        davres.change_properties(properties,
                                 delmark=zeit.cms.interfaces.DeleteProperty,
                                 locktoken=locktoken)
    ...
    def lock(self, id, principal, until):
        ...
        try:
            ...
            raise zeit.cms.interfaces.LockingError("%s is already locked." % id)
...
#                                                    % ("UNKNOWN", id))
    ...
    def _internal_add(self, id, resource):
        ...
        davres.change_properties(resource.properties,
                                 delmark=zeit.cms.interfaces.DeleteProperty,
                                 locktoken=locktoken)
        if autolock:
            self.unlock(id)
...
class ResourceCache(persistent.Persistent):
    ...
    zope.component.adapts(zope.app.component.interfaces.ISite)

    properties = zeit.cms.content.property.TransactionBoundCache(
        '_v_properties', dict)
    child_ids = zeit.cms.content.property.TransactionBoundCache(
        '_v_childnames', dict)
