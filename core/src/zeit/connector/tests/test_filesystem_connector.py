...
class MetadataTest(zeit.connector.testing.FilesystemConnectorTest):
    def test_minimal_properties_if_neither_metadata_head_nor_file(self):
        ...
        self.assertEqual([('type', 'http://namespaces.zeit.de/CMS/meta')], sorted(feed.properties))
    ...
    def test_metadata_read_from_metadata_file(self):
        ...
        author = image.properties[('author', 'http://namespaces.zeit.de/CMS/document')]
        self.assertEqual('Jochen Stahnke', author)
        banner = image.properties[('banner', 'http://namespaces.zeit.de/CMS/document')]
    ...
    def test_metadata_read_from_content_file_as_fallback(self):
        self.assertFalse(
            os.path.exists(
                os.path.join(
                    self.connector.repository_path, 'online/2007/01/4schanzentournee-abgesang.meta'
                )
            )
        )
        article = self.connector['http://xml.zeit.de/online/2007/01/4schanzentournee-abgesang']
        author = article.properties[('author', 'http://namespaces.zeit.de/CMS/document')]
        self.assertEqual(' Ulrich Dehne', author)  # space is from live data
        banner = article.properties[('banner', 'http://namespaces.zeit.de/CMS/document')]
        self.assertEqual('yes', banner)

    def test_metadata_for_metadata_file_itself_is_unknown(self):
        meta = self.connector['http://xml.zeit.de/2007/03/group.meta']
        type_ = meta.properties[('type', 'http://namespaces.zeit.de/CMS/meta')]
