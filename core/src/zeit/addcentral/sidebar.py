...
class Form(zope.formlib.form.SubPageForm):
    ...
    form_fields = zope.formlib.form.FormFields(zeit.cms.content.interfaces.IContentAdder).omit(
        'year', 'month'
    )
    prefix = 'sidebar.form'
    ...
    def setUpWidgets(self, ignore_request=False):
        session = zope.session.interfaces.ISession(self.request).get('zeit.addcentral', {})
    ...
    def add(self, action, data):
        ...
        self.next_url = adder()
        zope.session.interfaces.ISession(self.request)['zeit.addcentral'].update(data)
